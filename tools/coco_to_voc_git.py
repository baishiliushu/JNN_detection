...
anno = "instances_train2017.json"
xmldir = "train_voc_bbb/"
...
def json2xml():
    ...
    for im in imgs:
        ...
        for index, row in annos.iterrows():
            ...
            if (int(bbox[2])) < 2:
                if cate_name not in useless_kind.keys():
                    useless_kind[cate_name] = 1
                else:
                    useless_kind[cate_name] += 1
                continue
            if (int(bbox[3])) < 2:
        ...
        if object_count < 1:
            print("{} has no object".format(filename))
            continue
        ...
        f.close()
    print("lenth: {}\n{}".format(len(useless_kind.keys(), useless_kind)))
    sorted_values = sorted(useless_kind.items(), key=lambda x:x[1])
    for u in sorted_values:
        if sorted_values[u] > 1:
            print("{}:{}".format(u, sorted_values[u]))
