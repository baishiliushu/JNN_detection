...
from loss.loss import build_target_cls, yolo_loss_cls
...
class DarkJNNCls(nn.Module):

    def __init__(self, class_names=["neg", "pos"]):
        super(DarkJNNCls, self).__init__()
        self.class_number = 0
        ...
        self.conv7 = nn.Sequential(conv_bn_leaky(256 + 1024, 1024, kernel_size=3, return_module=True),
                                   nn.Conv2d(1024, (5 + self.class_number) * len(Config.anchors), kernel_size=1))  # (coords + conf) * anchors

        self.reorg = ReorgLayer()

    def forward(self, query, target, target_boxes=[], gt_classes=None, num_boxes=None, training=False):
        ...
        output = output.permute(0, 2, 3, 1).contiguous().view(bsize, h * w * len(Config.anchors), 5 + self.class_number)
        ...
        class_pred = F.softmax(class_score, dim=2)
        ...
        if training:
            ...
            gt_data = (target_boxes, gt_classes, num_boxes)
            target_data = build_target_cls(output_data, gt_data, h, w)

            target_variable = [Variable(v) for v in target_data]
            box_loss, iou_loss, class_loss = yolo_loss_cls(output_variable, target_variable)
